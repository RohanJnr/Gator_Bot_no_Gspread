...
class DiscordAgeCheck(commands.Cog):
    ...
    async def member_days_check(self, ctx):
        ...
        embed.title = 'Members who have stayed in the server for more than 90 days !'
        message = ''
    ...
    async def trials_days_check(self, ctx):
        ...
        embed = discord.Embed()
        embed.colour = 0x01d277
        embed.title = 'Trials who have stayed in the server for more than 14 days !'
        message = ''
        for member in member_role.members:
            today = datetime.datetime.now()
            days_stayed = (today - member.joined_at).days
